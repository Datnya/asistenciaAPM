...
export const startAttendanceSchema = z.object({
  ...
  workDate: z.string().date("Selecciona una fecha de asistencia válida."),
  workType: z.enum(["remote", "onsite"], "Selecciona el tipo de jornada."),
...
export const historicalAttendanceSchema = startAttendanceSchema.extend({
  exitTime: businessTimeSchema,
  declaredHours: z.string().refine(
    (value) => {
      const minutes = declaredHoursToMinutes(value);
